...
global locks
locks={}
logging_lib.basicConfig(filename='filesystem.log', level=logging_lib.INFO, format='%(asctime)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S')

running = True
path='C:/Users/Chaimama/Desktop/Pcs_Project/cmsc626distributed-file-system-main/'
    ...
    class CoreThread(Thread): 
        ...
        def run(self): 
            ...
            if request == "createdir":
                ...
                if os.path.exists(path+user) == False:
                    os.mkdir(path+user)
                if os.path.exists(path+user+"/"+dirname) == False:
                    os.mkdir(path+user+"/"+dirname)
            ...
            if request == "createfile":
                ...
                if os.path.exists(path+user+"/"+dirname):
                    open(path+user+"/"+dirname+"/"+filename, 'w').close()
            ...
            if request == "deletefile":
                ...
                filename = decrypt(filename)
                filename = filename.decode("utf-8")
                path_to_check = file_path_variable = r'C:\Users\Chaimama\Desktop\Pcs_Project\cmsc626distributed-file-system-main\{}\{}\{}'.format(user, dirname, filename)
                if os.path.exists(path_to_check):
                     if os.path.isdir(path+user+"/"+dirname+"/"+filename):
                        shutil.rmtree(path+user+"/"+dirname+"/"+filename)
                if os.path.isfile(path_to_check): 
                        os.remove(path+user+"/"+dirname+"/"+filename)
                        conn.send("Given file deleted".encode())
                        print("Given file deleted: "+filename)
                else:
            ...
            if request == "writefile":
                ...
                if os.path.exists(path+user+"/"+dirname+"/"+filename):
                    f = open(path+user+"/"+dirname+"/"+filename, "w")
                    f.write(encrypt)
                    f.close()
            ...
            if request == "renamefile":
                ...
                if os.path.exists(path+user+"/"+dirname+"/"+oldname):
                    os.rename(path+user+"/"+dirname+"/"+oldname,path+user+"/"+dirname+"/"+newname)
            ...
            if request == "listfiles":
                user = dataset[1]
                file_list = []
                for root, dirs, directory in os.walk(path+user):
            ...
            if request == "readfile":
                ...
                features = []
                if os.path.exists(path+user+"/"+dirname+"/"+filename):
                    with open(path+user+"/"+dirname+"/"+filename) as f:
                        data = f.read()
                    f.close()
